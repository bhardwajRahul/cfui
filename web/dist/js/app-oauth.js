...
(() => {
    ...
    const oauthSetup = window.cfui.oauthSetup.create({
        ...
        fullConsoleScopes: oauthFullConsoleSetupScopes,
    });
    ...
    async function loadOAuthZones() {
        ...
        try {
            ...
            state.oauth.zonesSession = resp.session || state.oauth.zonesSession || null;
            state.oauth.zonesCapabilities = resp.capabilities || state.oauth.zonesCapabilities || null;
            ...
            state.oauth.zones = [];
            state.oauth.zonesError = err.message || String(err);
            renderOAuthError(err.message);
        }
    }
    ...
    function renderOAuthStatus(status) {
        ...
        if (relay) {
            relay.innerHTML = '';
            relay.appendChild(oauthSetup.relayCallbackNode(status));
        }
        ...
        oauthSetup.renderSetupGuide(status);
    ...
    function zonesDiagnosticsText() {
        const status = state.oauth.status || {};
        const session = state.oauth.zonesSession || state.oauth.zoneDetail?.session || status.current || {};
        const selected = selectedZoneDetail();
        return JSON.stringify({
            type: 'cfui_oauth_zones_diagnostics',
            version: 1,
            generated_at: new Date().toISOString(),
            browser_origin: window.location.origin,
            browser_path: window.location.pathname,
            ...
            capability: {
                zones_read: canRead('zones'),
                dns_read: canRead('dns'),
                dns_write: canWrite('dns'),
            },
            state: {
                zones_loaded: state.oauth.zones.length,
                zones_error: state.oauth.zonesError || '',
                detail_loaded: !!selected,
                detail_error: state.oauth.zoneDetailError || '',
                detail_loading: !!state.oauth.zoneDetailLoading,
                dns_count_loaded: state.oauth.zoneDNSCount != null,
                dns_count_value: state.oauth.zoneDNSCount == null ? null : Number(state.oauth.zoneDNSCount),
                dns_count_error: state.oauth.zoneDNSCountError || '',
                dns_count_loading: !!state.oauth.zoneDNSCountLoading,
                dns_count_zone_id: state.oauth.zoneDNSCountZoneId || '',
                scope_ready: canRead('zones'),
            },
            zones: state.oauth.zones.map(zoneDiagnostics),
            selected_zone: selected ? zoneDiagnostics(selected) : null,
            capabilities: oauthCapabilityDiagnostics(state.oauth.zonesCapabilities || state.oauth.zoneDetail?.capabilities || status.capabilities || {}),
        }, null, 2);
    }

    function zoneDiagnostics(zone) {
        return {
            id: zone?.id || '',
            name: zone?.name || '',
            status: zone?.status || '',
            type: zone?.type || '',
            paused: !!zone?.paused,
            account_id: zone?.account_id || zone?.account?.id || '',
            account_name: zone?.account?.name || '',
            plan_name: zone?.plan?.name || '',
            plan_legacy_id: zone?.plan?.legacy_id || '',
            plan_frequency: zone?.plan?.frequency || '',
            name_servers: Array.isArray(zone?.name_servers) ? zone.name_servers : [],
            original_name_servers_count: Array.isArray(zone?.original_name_servers) ? zone.original_name_servers.length : 0,
    ...
    function renderZones(body) {
        body.appendChild(resourceActionBar(t('oauth_zones'), {
            text: t('oauth_zones_copy_diagnostics'),
            className: 'btn btn--sm btn--ghost',
            title: t('oauth_zones_copy_diagnostics_title'),
            onClick: () => copyOAuthText(zonesDiagnosticsText()),
        }));
        if (state.oauth.zonesError) {
            body.appendChild(empty(state.oauth.zonesError));
            return;
        }
        if (!state.oauth.zones.length) {
    ...
    function kvDiagnosticsText() {
        ...
        const filteredFolders = filteredKVPrefixes(entries.folders);
        const filteredKeys = filteredKVKeys(entries.keys);
        return JSON.stringify({
            ...
            selected: {
                ...
                key_filter: String(state.oauth.kvKeyFilter || '').trim(),
                storage_view: state.oauth.storageView || '',
            },
            identity: {
                label: session.label || '',
            ...
            state: {
                ...
                key_filter: String(state.oauth.kvKeyFilter || '').trim(),
                key_filter_active: !!String(state.oauth.kvKeyFilter || '').trim(),
                key_filter_matches: filteredFolders.length + filteredKeys.length,
            ...
            keys: {
                ...
                filtered_count: filteredFolders.length + filteredKeys.length,
                prefixes: filteredFolders.map(kvPrefixDiagnostics),
                has_more: !!state.oauth.kvCursor,
                cursor_present: !!state.oauth.kvCursor,
                items: filteredKeys.map(kvKeyDiagnostics),
            },
    ...
    function renderKVDetail(body) {
        ...
        section.appendChild(kvKeyFilterNode());
        const entries = kvKeyEntries();
        const filteredFolders = filteredKVPrefixes(entries.folders);
        const filteredKeys = filteredKVKeys(entries.keys);
        ...
        } else if (!filteredFolders.length && !filteredKeys.length) {
            section.appendChild(empty(t('oauth_kv_no_key_matches')));
        } else {
            if (canEdit) section.appendChild(kvBulkActionsNode(filteredKeys));
    ...
    function kvKeyFilterNode() {
        const wrap = document.createElement('div');
        wrap.className = 'oauth-filter-bar';
        const input = textInput(state.oauth.kvKeyFilter || '', 'search');
        input.setAttribute('data-oauth-kv-key-filter', 'true');
        input.placeholder = t('oauth_kv_key_search_placeholder');
        input.setAttribute('aria-label', t('oauth_kv_key_search_label'));
        input.autocomplete = 'off';
        input.spellcheck = false;
        input.addEventListener('input', () => {
            state.oauth.kvKeyFilter = input.value;
            renderOAuthResource();
            requestAnimationFrame(() => {
                const next = document.querySelector('[data-oauth-kv-key-filter="true"]');
                if (!next) return;
                next.focus();
                const end = next.value.length;
                next.setSelectionRange(end, end);
        ...
        const meta = document.createElement('div');
        meta.className = 'oauth-filter-meta';
        meta.textContent = t('oauth_kv_key_search_count', {
            n: filteredKVPrefixes(entries.folders).length + filteredKVKeys(entries.keys).length,
            m: entries.folders.length + entries.keys.length,
        });
        wrap.append(input, meta);
        return wrap;
    }
    ...
    function filteredKVKeys(keys = kvDirectKeys()) {
        const query = normalizeFilterText(state.oauth.kvKeyFilter);
        if (!query) return keys;
        return keys.filter((key) => kvKeySearchText(key).includes(query));
    }

    function filteredKVPrefixes(folders = kvKeyEntries().folders) {
        const query = normalizeFilterText(state.oauth.kvKeyFilter);
        if (!query) return folders;
        return folders.filter((folder) => normalizeFilterText([folder?.prefix, kvPrefixLabel(folder?.prefix)].join(' ')).includes(query));
    }
    ...
    function kvKeySearchText(key) {
        const expiration = Number(key?.expiration || 0);
        return normalizeFilterText([
            key?.name,
            expiration ? formatDate(expiration * 1000) : '',
        ].filter(Boolean).join(' '));
    }
    ...
    function wireOAuth() {
        ...
        $('oauth-worker-script-copy')?.addEventListener('click', async () => {
            if (!state.oauth.workerScriptContent && !state.oauth.workerScriptLoading) await oauthSetup.loadWorkerScript();
            copyOAuthText(state.oauth.workerScriptContent || '');
        });
        $('oauth-worker-script-open')?.addEventListener('click', oauthSetup.openWorkerScript);
