...
- OAuth 未配置时，Cloudflare 工作台展示单个 relay callback 输入框；默认中转域名和自建 Worker relay 作为输入框下方的轻量文本动作出现，其中自建 Worker 只打开弹窗显示“提供的中转 / 自建中转”的取舍、脚本和部署步骤。引导用户创建 OAuth app 时只强调一个 Cloudflare Redirect callback URL，不展示 `cfui_callback_url` 参数。
