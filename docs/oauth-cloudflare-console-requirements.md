...
- OAuth 未配置时，Cloudflare 工作台展示单个 relay callback 输入框；输入框旁提供复制、保存和检测动作，检测会通过 `/api/oauth/relay-check` 验证 relay `/health` 是否支持 `state-v1`。输入框下方显示当前值来源（提供域名、自定义或未保存），“使用提供域名”和“自建说明”作为轻量文本动作出现，其中“使用提供域名”会填入并保存提供的中转地址，“自建说明”只打开弹窗显示目标回调 URL 格式、自建步骤和脚本。引导用户创建 OAuth app 时只强调一个 Cloudflare Redirect callback URL，不展示 `cfui_callback_url` 参数。
...
- D1 数据库创建/删除、详情（file size / table count）回填、SQL 查询控制台、table 浏览、分页行加载、按列类型显示数字/布尔/日期时间/文本输入的行编辑器、参数化更新/删除；前端可复制不含 OAuth/refresh token、SQL 文本、SQL 参数、查询结果行和表格行数据的 D1 诊断快照。
...
| Storage / D1 | Database list, SQL console | Database list/create/delete/detail, SQL console, table browser, type-aware row editor, row update/delete, and no-token/no-row-data diagnostics export | Optional location/jurisdiction fields if exposed through the pinned SDK or a deliberate REST path |
