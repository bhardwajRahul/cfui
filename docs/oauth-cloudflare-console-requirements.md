...
- OAuth 未配置时，Cloudflare 工作台展示单个 relay callback 输入框；输入框旁提供复制、保存和检测动作，检测会通过 `/api/oauth/relay-check` 验证 relay `/health` 是否支持 `state-v1`。“使用默认”和“自建 Worker”作为输入框下方的轻量文本动作出现，其中“使用默认”会填入并保存提供的中转地址，“自建 Worker”只打开弹窗显示简短说明、自建步骤和脚本。引导用户创建 OAuth app 时只强调一个 Cloudflare Redirect callback URL，不展示 `cfui_callback_url` 参数。
...
- 账号、带套餐/名称服务器详情的 Zone、DNS、Cloudflare Tunnel control-plane（状态、类型、remote config、活跃连接明细）、Workers、R2、D1、KV、Snippets、WAF、自选 Zone settings、Cloudflare 官方状态的资源面。Cloudflare Go SDK 路径和手写 REST 路径都统一支持后端 endpoint override，便于 fake Cloudflare 端到端验证；账号列表显式按 SDK `ResultInfo.TotalPages` 自动翻页，不只显示第一页；Account 列表、Zone 列表和所选详情可复制不含 OAuth/refresh token 的诊断快照。
...
- Zone settings 写操作：`zone-settings.write` 控制 SSL/TLS mode、development mode、security level、cache level、browser cache TTL、Always Use HTTPS、Automatic HTTPS Rewrites、Brotli、Rocket Loader；`cache_purge.write` 单独控制全量 cache purge，后端 capability 兼容识别旧的 `cache.purge`；前端可复制不含 OAuth/refresh token 的 Zone Settings 诊断快照。
