...
| Analytics | Zone traffic via GraphQL, 24h/7d/30d | Zone analytics summary with copyable export text and request trend bars, single Worker metrics, account usage dashboard with GraphQL plus R2 REST metrics and no-token diagnostics export | Deeper visual chart polish only if real accounts show the compact bars are insufficient |
