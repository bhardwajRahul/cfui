...
- OAuth 未配置时，Cloudflare 工作台只突出单个 relay callback 输入框；输入框旁提供复制、按需出现的保存动作和低干扰检测动作，检测会通过 `/api/oauth/relay-check` 验证 relay `/health` 是否支持 `state-v1`。输入框下方只保留一行轻量提示和文字动作：“用提供域名”会填入并保存提供的中转地址，“自建”只打开弹窗，先说明提供域名与自建 Worker 两条路径，再显示自建 Worker 的目标回调 URL、自建步骤和脚本；来源状态与检测结果合并为一行反馈。本地 cfui callback 不在未配置引导中突出展示。引导用户创建 OAuth app 时只强调一个 Cloudflare Redirect callback URL，不展示 `cfui_callback_url` 参数。
...
- Cloudflare Tunnel 支持通过 OAuth 创建 remote-managed tunnel，后端生成 tunnel secret 并读取 connector token；connector token 不返回前端，只在用户勾选时写入本地 SQLite tunnel profile。OAuth 隧道列表会返回无密钥的本地 profile 摘要，前端能标记远端 tunnel 已关联到哪个本地 profile，并在 ingress 面板提供打开本地工作台的 handoff；删除远端 tunnel 时可同步清理关联的本地 profile；本地 `/local` tunnel profile 列表也会显示对应 Cloudflare Tunnel 的短 ID。OAuth 工作台可读取、创建、编辑、删除和拖拽排序 Cloudflare-hosted tunnel ingress/public hostname 规则，排序写回 Cloudflare Tunnel configuration，并保护 catch-all 规则保留在最后。
