...
- KV namespace 创建/重命名/删除，key 列表、已加载 key 批量删除、UTF-8 value 查看/编辑/删除、文本 key 创建；非 UTF-8 binary value 返回受限 hexdump 样本预览，仍避免在文本编辑器里误写二进制。
...
| Storage / KV | Namespace/key list, value create/edit/delete | Namespace create/rename/delete, key list, loaded-key bulk delete, UTF-8 value create/edit/delete, binary value hexdump preview | Optional binary upload/download workflows if Cloudflare KV OAuth use cases need raw bytes editing |
