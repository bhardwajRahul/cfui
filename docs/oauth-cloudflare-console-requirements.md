...
- OAuth 未配置时，Cloudflare 工作台展示单个 relay callback 输入框、默认中转域名的小按钮、自建 Worker relay 弹窗、可编辑 relay callback、local callback、relay 自检按钮和 cfui 环境变量，引导用户创建 OAuth app 或自部署 relay Worker。
