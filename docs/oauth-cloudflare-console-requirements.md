...
- OAuth login/callback/logout/status/config，使用 Worker relay，token 和 PKCE state 存 SQLite；OAuth client ID 可通过环境变量或 WebUI 保存到 SQLite，环境变量优先；登录前可按已实现模块选择本次授权 scopes，WebUI 默认从基础读权限开始，`CFUI_OAUTH_SCOPES` 作为直接访问 `/oauth/start` 时的兼容 fallback；scope selector 可复制不含 OAuth token 的配置模板、下一次请求 scope、当前身份实际授权 scope 和功能读写能力矩阵。
- 可自部署的 Worker relay 脚本放在 `docs/cloudflare-oauth-worker.js`。Cloudflare OAuth Client 的 Redirect URL 固定填写 Worker 的公网 HTTPS 回调，例如 `https://cfoauth.pushcat.eu.org/oauth/callback`，不要追加 `cfui_callback_url`。cfui 在发起登录时把当前浏览器访问到的 `/oauth/callback` URL 编码进 OAuth `state`；Worker 从 state 解析目标后，把 `code`、`state` 或 OAuth 错误转发回对应 cfui 实例。Worker 变量 `CFUI_CALLBACK_URL` 仅作为 fallback；一个 Worker 服务公网 cfui 域名时，通过 `CFUI_ALLOWED_CALLBACK_ORIGINS` 配置来源白名单，或明确设置 `*` 作为多用户 relay。Worker 提供 `/health`，cfui 通过 `/api/oauth/relay-check` 从 relay callback URL 推导并检查该健康端点。relay callback 可在 WebUI 保存到 SQLite，保存值覆盖 `CFUI_OAUTH_RELAY_URL`，并要求 URL 为 `http/https`、有 host、路径为 `/oauth/callback`。
- OAuth 未配置时，Cloudflare 工作台突出 OAuth Client ID 与 relay callback 两个输入框；顶部提示带“去设置”入口并聚焦缺失项。配置引导未配置时默认展开，配置完成后默认收起但保留切换按钮。relay 输入框同一行只保留复制和按需出现的保存动作，检测动作保留在单行状态里并通过 `/api/oauth/relay-check` 验证 relay `/health` 是否支持 `state-v1`。输入框下方只保留一句轻量提示、“默认”和“自建”文字入口：“默认”会填入并保存提供域名对应的 OAuth Client ID `27efa323c59c87976675dc75bcad907b` 和中转地址 `https://cfoauth.pushcat.eu.org/oauth/callback`，“自建”只打开弹窗，先说明可以直接使用提供域名，也可以自建 Worker，再显示自建 Worker 的目标回调 URL、自建步骤和脚本；来源状态与检测结果合并为一行反馈。本地 cfui callback 不在未配置引导中突出展示。引导用户创建 OAuth app 时只强调一个 Cloudflare Redirect callback URL，不展示 `cfui_callback_url` 参数。
- OAuth Cloudflare console 初版 UI，包含对齐 orange-cloud 首页体验的 Overview dashboard；Overview 由后端聚合账号、Zone、DNS、Workers、Tunnel、R2、D1、KV、Snippets、WAF 和 Cloudflare 公开状态，scope 缺失或单个产品 API 失败时只标记对应指标不可用，并可复制不含 OAuth token 的诊断快照用于真实账号/scope 组合回归。Overview 还提供无 token 校验工作台，复用只读 API smoke 区分 OAuth app 权限白名单、当前请求/授权 scope、Cloudflare API/套餐限制、资源为空和公共状态问题；报告前端按 Identity、Zone/DNS/Settings、Tunnel、Workers、R2、D1、KV、Snippets、WAF、Analytics、Status 分组显示 scope/API 状态和处理建议；本地 SQLite 校验归档的列表摘要包含请求 scope 与实际授权 scope 的数量、短 fingerprint、问题数量和 action item 数量，便于对比不同 OAuth app allowlist / granted-scope 组合；本地校验历史可在未登录时读取，方便登录失效后继续查看历史结果，且不包含 OAuth/refresh token、connector token、DNS content、R2 object body、KV value、SQL rows 或 WAF expression。
...
- Cloudflare Tunnel 支持通过 OAuth 创建 remote-managed tunnel，后端生成 tunnel secret 并读取 connector token；connector token 不返回前端，只在用户勾选时写入本地 SQLite tunnel profile。OAuth 隧道列表会返回无密钥的本地 profile 摘要，前端能标记远端 tunnel 已关联到哪个本地 profile，并在 ingress 面板提供打开本地工作台的 handoff；handoff 使用 `/local?tunnel_key=<profile>` 直接选中对应本地 tunnel profile，随后清理临时 URL 参数；Tunnel 诊断快照会包含不带 token 的 handoff path/URL，便于真实账号排查本地 runner 与远端控制面关联；删除远端 tunnel 时可同步清理关联的本地 profile；本地 `/local` tunnel profile 列表也会显示对应 Cloudflare Tunnel 的短 ID。OAuth 工作台可读取、创建、编辑、删除和拖拽排序 Cloudflare-hosted tunnel ingress/public hostname 规则，排序写回 Cloudflare Tunnel configuration，并保护 catch-all 规则保留在最后。
...
- R2 bucket 创建/删除，使用 Cloudflare Go SDK bucket 级 API，按 `workers-r2.write` scope 显示；R2 object 列表支持 Cloudflare R2 `prefix` + `delimiter` 的目录式浏览、已加载对象/目录本地搜索、复制 key、详情元信息面板、受限 UTF-8 预览、非 UTF-8 对象的受限 hexdump 样本预览、50 MiB 内常见图片/音频/视频/PDF 内联预览、文本 object 创建/覆盖、同账号同 bucket/跨 bucket object 复制/移动、128 MiB 内直接文件上传、5 GiB 内浏览器分片上传到 cfui 临时文件后由后端流式写入 R2、同源文件下载、删除和账号级 metrics 通过后端 Cloudflare REST API 实现。OAuth token 只在后端使用，R2 metrics 按需读取，不持久化快照；前端可复制不含 OAuth/refresh token、对象正文、binary preview 和下载 URL 的 R2 诊断快照。
- KV namespace/key 管理使用 Cloudflare Go SDK；key 列表通过 `prefix` 参数加载，并在前端按 `/` 推导目录式浏览，支持 root/当前 prefix 面包屑、目录行、当前已加载目录/key 本地搜索、当前层 key 批量选择、创建/上传默认落到当前 prefix；KV 诊断快照包含 prefix、搜索条件和目录/key 列表摘要，但不包含 KV value、binary preview 或下载 URL。
...
| Storage / R2 | Bucket/object browsing | Bucket list/create/delete, prefix/delimiter folder-style object browsing, object search/preview/create/direct upload/chunked large upload/copy/download/delete, account metrics, and no-token/no-object-content diagnostics export | Native R2 multipart only if OAuth REST exposes it or users provide S3 credentials |
