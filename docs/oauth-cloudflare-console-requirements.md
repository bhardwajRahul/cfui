...
- OAuth 未配置时，Cloudflare 工作台展示单个 relay callback 输入框；“使用默认”和“自建 Worker”作为输入框下方的轻量文本动作出现，其中“使用默认”会填入并保存提供的中转地址，“自建 Worker”只打开弹窗显示简短说明、自建步骤和脚本。引导用户创建 OAuth app 时只强调一个 Cloudflare Redirect callback URL，不展示 `cfui_callback_url` 参数。
...
- D1 数据库创建/删除、详情（file size / table count）回填、SQL 查询控制台、table 浏览、分页行加载、参数化更新/删除；前端可复制不含 OAuth/refresh token、SQL 文本、SQL 参数、查询结果行和表格行数据的 D1 诊断快照。
...
| Domains / DNS | Zone list/detail, DNS CRUD, proxy toggle, zone settings | Zone list/detail, plan/name servers, DNS count/list/search/create/update/delete/proxy toggle, no-token/no-record-content diagnostics export, selected zone settings, cache purge | Broader settings catalog only if Cloudflare OAuth scopes allow it |
...
| Storage / D1 | Database list, SQL console | Database list/create/delete/detail, SQL console, table browser, row update/delete, and no-token/no-row-data diagnostics export | Optional location/jurisdiction fields if exposed through the pinned SDK or a deliberate REST path |
