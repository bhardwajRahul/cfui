...
- OAuth 未配置时，Cloudflare 工作台展示单个 relay callback 输入框；默认中转域名、自建 Worker relay 和 relay 自检都作为轻量文本动作出现，其中自建 Worker 只打开弹窗显示脚本、callback URL 格式和部署步骤。引导用户创建 OAuth app 时只强调一个 Cloudflare Redirect callback URL，不展示 `cfui_callback_url` 参数。
- OAuth Cloudflare console 初版 UI，包含对齐 orange-cloud 首页体验的 Overview dashboard；Overview 由后端聚合账号、Zone、DNS、Workers、Tunnel、R2、D1、KV、Snippets、WAF 和 Cloudflare 公开状态，scope 缺失或单个产品 API 失败时只标记对应指标不可用，并可复制不含 OAuth token 的诊断快照用于真实账号/scope 组合回归。
...
- Zone settings 写操作：`zone-settings.write` 控制 SSL/TLS mode、development mode、security level、cache level、browser cache TTL、Always Use HTTPS、Automatic HTTPS Rewrites、Brotli、Rocket Loader；`cache_purge.write` 单独控制全量 cache purge，后端 capability 兼容识别旧的 `cache.purge`。
...
- Workers 列表和详情：通过 Cloudflare Go SDK 读取脚本 metadata、settings、Tail consumers 和受大小限制的只读脚本内容预览；当前不提供 Worker 脚本部署/编辑 UX，因此 capability matrix 和 scope 选择器不报告或申请 `workers-scripts.write`。
...
- KV namespace 创建/重命名/删除，key 列表、已加载 key 批量删除、UTF-8 value 查看/编辑/删除、文本 key 创建；非 UTF-8 binary value 返回受限 hexdump 样本预览，并可通过同源后端代理上传/下载原始字节，仍避免在文本编辑器里误写二进制。
- D1 数据库创建/删除、详情（file size / table count）回填、SQL 查询控制台、table 浏览、分页行加载、参数化更新/删除。
- Snippets 创建/删除、既有代码正文读取/编辑、触发规则列表/新增/启停/删除；代码正文通过后端低层 Cloudflare REST `/content` endpoint 读取，写入复用 Cloudflare Go SDK multipart update，规则更新采用 read-modify-write 整组回写。Snippet rule 表达式编辑器带常见 host/path/method/IP/country 片段 helper，可追加或替换现有表达式。
...
| Analytics | Zone traffic via GraphQL, 24h/7d/30d | Zone analytics summary with copyable export text and request trend bars, single Worker metrics, account usage dashboard with GraphQL plus R2 REST metrics | Deeper visual chart polish only if real accounts show the compact bars are insufficient |
| Workers | Script list/detail, read-only detail, live tail | Worker list/detail, script preview, settings, tail consumers, worker metrics, backend SSE live tail proxy; `workers-scripts.write` is intentionally not requested or reported until a safe deploy UX exists | Real-account validation of read-only script preview, metrics, and tail behavior |
| Snippets | View/edit/create snippets and trigger rules | Snippet list/create/delete, content read/write, rules create/enable/disable/delete, common expression helper for host/path/method/IP/country matching | Real-account tuning for complex matching expressions |
...
| Storage / KV | Namespace/key list, value create/edit/delete | Namespace create/rename/delete, key list, loaded-key bulk delete, UTF-8 value create/edit/delete, binary value hexdump preview/upload/download | Optional raw byte editor only if real KV use cases need in-browser binary editing |
| Security / WAF | Custom WAF view/toggle | Custom WAF create/edit/enable/disable/delete plus managed overrides/exceptions, advanced JSON preservation, rate-limit builder, skip parameter builder, and common expression helper | Real-account validation of expression presets and managed ruleset edge cases |
| Network / Tunnel | Tunnel status and public hostnames | Tunnel list/details/connections, create remote-managed tunnel, link local profile without exposing connector token, expose linked local runner status, delete remote tunnel with optional local profile cleanup, public hostname/ingress CRUD and drag reorder with local connector follow-up hints | Real-account validation for how quickly running cloudflared connectors pick up remote config changes |
