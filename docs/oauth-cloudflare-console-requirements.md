...
- OAuth login/callback/logout/status/config，使用 Worker relay，token 和 PKCE state 存 SQLite；OAuth client ID 可通过环境变量或 WebUI 保存到 SQLite，环境变量优先；登录前可按已实现模块选择本次授权 scopes，`CFUI_OAUTH_SCOPES` 作为默认模板和兼容 fallback；scope selector 可复制不含 OAuth token 的配置模板、下一次请求 scope、当前身份实际授权 scope 和功能读写能力矩阵。
