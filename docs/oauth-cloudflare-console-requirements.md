...
- 账号、带套餐/名称服务器详情和 orange-cloud 常用快捷操作的 Zone、DNS、Cloudflare Tunnel control-plane（状态、类型、remote config、活跃连接明细）、Workers、R2、D1、KV、Snippets、WAF、自选 Zone settings、Cloudflare 官方状态的资源面。Cloudflare Go SDK 路径和手写 REST 路径都统一支持后端 endpoint override，便于 fake Cloudflare 端到端验证；账号列表显式按 SDK `ResultInfo.TotalPages` 自动翻页，不只显示第一页；Account 列表、Zone 列表和所选详情可复制不含 OAuth/refresh token 的诊断快照。
...
- Zone settings 写操作：`zone-settings.write` 控制 SSL/TLS mode、development mode、security level、cache level、browser cache TTL、Always Use HTTPS、Automatic HTTPS Rewrites、Brotli、Rocket Loader；Zone 详情页会把 development mode、security level 和全量 cache purge 下沉成快捷操作；`cache_purge.write` 单独控制全量 cache purge，后端 capability 兼容识别旧的 `cache.purge`；前端可复制不含 OAuth/refresh token 的 Zone Settings 诊断快照。
