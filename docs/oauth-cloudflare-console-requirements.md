...
- OAuth 未配置时，Cloudflare 工作台只突出单个 relay callback 输入框；输入框同一行只保留复制和按需出现的保存动作，检测动作保留在单行状态里并通过 `/api/oauth/relay-check` 验证 relay `/health` 是否支持 `state-v1`。输入框下方只保留一句轻量提示、“默认”和“自建”文字入口：“默认”会填入并保存提供的中转地址，“自建”只打开弹窗，先说明可以直接使用提供域名，也可以自建 Worker，再显示自建 Worker 的目标回调 URL、自建步骤和脚本；来源状态与检测结果合并为一行反馈。本地 cfui callback 不在未配置引导中突出展示。引导用户创建 OAuth app 时只强调一个 Cloudflare Redirect callback URL，不展示 `cfui_callback_url` 参数。
