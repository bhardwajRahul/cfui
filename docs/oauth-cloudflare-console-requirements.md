...
- OAuth 未配置时，Cloudflare 工作台展示“使用提供的 relay 地址”与“自部署 Worker relay”两条路径、可编辑 relay callback、local callback、页面内完整 Worker 脚本查看/复制入口、Worker 变量、relay 自检按钮和 cfui 环境变量，引导用户创建 OAuth app 或自部署 relay Worker。
...
- KV namespace 创建/重命名/删除，key 列表、已加载 key 批量删除、UTF-8 value 查看/编辑/删除、文本 key 创建；非 UTF-8 binary value 返回受限 hexdump 样本预览，并可通过同源后端代理下载原始字节，仍避免在文本编辑器里误写二进制。
...
| Storage / KV | Namespace/key list, value create/edit/delete | Namespace create/rename/delete, key list, loaded-key bulk delete, UTF-8 value create/edit/delete, binary value hexdump preview/download | Optional binary upload/raw-edit workflows if Cloudflare KV OAuth use cases need raw bytes editing |
