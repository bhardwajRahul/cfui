...
- Cloudflare Tunnel 支持通过 OAuth 创建 remote-managed tunnel，后端生成 tunnel secret 并读取 connector token；connector token 不返回前端，只在用户勾选时写入本地 SQLite tunnel profile。OAuth 隧道列表会返回无密钥的本地 profile 摘要，前端能标记远端 tunnel 已关联到哪个本地 profile；删除远端 tunnel 时可同步清理关联的本地 profile；本地 `/local` tunnel profile 列表也会显示对应 Cloudflare Tunnel 的短 ID。
...
| Storage / R2 | Bucket/object browsing | Bucket list/create/delete, object list/search/preview/create/upload/copy/download/delete, account metrics | Resumable/chunked large uploads |
| Storage / D1 | Database list, SQL console | Database list/detail, SQL console, table browser, row update/delete | Optional database create/delete after confirmation UX |
...
| Network / Tunnel | Tunnel status | Tunnel list/details/connections, create remote-managed tunnel, link local profile without exposing connector token, delete remote tunnel with optional local profile cleanup | OAuth workspace public hostname / ingress route configuration |
