...
OAuth mode calls Cloudflare APIs directly with the current OAuth identity's bearer token. It does not create API tokens on behalf of the user. cfui can store multiple OAuth identities in SQLite and rename, switch, or remove them from the OAuth console; no OAuth session or PKCE state is written to JSON. Before each sign-in, the OAuth console can build a module-based scope set for the next identity; when adding an identity while already signed in, cfui opens Cloudflare through the logout endpoint first so browser cookies do not silently reuse the current account. The WebUI starts new sign-ins from base read scopes and lets users explicitly add more scopes; `CFUI_OAUTH_SCOPES` remains the fallback for direct `/oauth/start` use. When OAuth is not configured, the Cloudflare workspace shows OAuth Client ID and relay callback inputs, a compact source/health line, a small action to save the provided Client ID plus relay domain, and a self-host dialog that first explains the provided-domain path versus the self-hosted Worker path before showing the target callback URL, full Worker script, and deployment notes. The setup guide is expanded until OAuth is configured, then collapses behind a toggle for later reference. The relay callback can be edited from the WebUI; the saved SQLite value overrides `CFUI_OAUTH_RELAY_URL` and must exactly match the Cloudflare OAuth client's redirect callback URL. Available Cloudflare pages are controlled by the scopes granted to the current OAuth app session; local MCP and S3 WebDAV remain in the local cfui workspace and are not shown inside `/cloudflare`.

The OAuth console currently exposes an orange-cloud-style Overview dashboard, accounts, zones with plan/name-server detail, DNS records, Cloudflare Tunnel control-plane records with status/type/remote config/active connection detail and linked local runner state, Workers scripts, R2 buckets, objects, and account-level metrics, D1 databases, KV namespaces, zone-level Snippets, custom WAF rules, managed WAF ruleset overrides, and managed WAF exceptions, Zone Analytics, Account Usage, Cloudflare public status, and selected zone settings. The Overview uses one backend endpoint to aggregate account, zone, DNS, Workers, Tunnel, R2, D1, KV, Snippets, WAF, and public status counts, then shows capability-aware quick actions plus a recent-zone panel that opens the selected zone or its DNS records; missing scopes or per-product API failures are shown as unavailable metrics instead of breaking the whole console. The Overview can also run `GET /api/cf/validation`, copy a no-token validation report, and save server-generated validation snapshots to local SQLite through `POST /api/cf/validation-reports`; saved summaries can be listed, opened, and deleted without uploading arbitrary report JSON from the browser. These reports separate OAuth app allowlist/requested/granted scope gaps from Cloudflare API availability failures and include structured action items, missing scope names, limited API checks, and unavailable API checks while omitting OAuth/refresh tokens, connector tokens, DNS content, object bodies, KV values, SQL rows, and WAF expressions. DNS records can be searched locally; A/AAAA/CNAME proxy state can be toggled from the list, and records can be created, updated, and deleted when `dns.write` is granted. Workers include list and detail views with script metadata, settings, Tail consumers, a bounded read-only script preview when `workers-scripts.read` is granted, per-script metrics through the GraphQL Analytics API when `analytics.read` or `account-analytics.read` is granted, and Live Tail streaming through a backend SSE proxy when `workers-tail.read` is granted. R2 buckets can be created and deleted when `workers-r2.write` is granted; bucket object lists, local search over loaded objects, bounded UTF-8 previews, bounded hexdump previews for non-UTF-8 objects, common image/audio/video/PDF previews through the same-origin download proxy, text object create/update, direct file upload up to 128 MiB, chunked browser-to-cfui upload up to 5 GiB followed by backend streaming to R2, same-origin file download, object delete, and account-level storage metrics are available through backend Cloudflare REST calls because the current Cloudflare Go SDK only exposes bucket-level R2 APIs. R2 metrics are read on demand and are not persisted as local snapshots. KV namespaces can be created, renamed, deleted, drilled into keys, used to view UTF-8 values, edit/delete values with `workers-kv-storage.write`, create text keys, upload/download raw value bytes through the same-origin backend proxy, and bulk delete loaded keys through Cloudflare's bulk KV API. D1 databases can be created and deleted with confirmation when `d1.write` is granted, include create-time jurisdiction / primary location hint / read replication options, and show placement metadata when Cloudflare returns it; D1 also includes a SQL query console plus table browsing, paged row loading, a type-aware row editor with explicit SQL NULL support for nullable columns, and parameterized row update/delete. Snippets can be created/deleted, existing snippet code can be loaded and edited through the backend `/content` REST endpoint, and trigger rules can be added, enabled, disabled, or deleted when `snippets.write` is granted. Custom WAF rules can be created, edited, enabled, disabled, or deleted when `zone-waf.write` is granted. The simple WAF editor supports the safe action subset (`block`, `challenge`, `managed_challenge`, `js_challenge`, `log`, and `skip`); `skip` rules support Cloudflare action parameters for current ruleset, products, and phases, and the form includes a dedicated rate-limit builder for common `ratelimit` fields. Existing and complex WAF rules can also be edited through an Advanced JSON section for `action_parameters`, `ratelimit`, `logging`, and `exposed_credential_check`; unchanged advanced JSON fields are not submitted, and `null` clears a field. WAF updates use read-modify-write and preserve untouched advanced fields. Managed WAF ruleset overrides are isolated to the `http_request_firewall_managed` phase and only create or edit `execute` rules with a managed ruleset ID plus ruleset, tag/category, and managed-rule override fields. Managed WAF exceptions are isolated to the same phase and only create or edit `skip` rules for the current managed ruleset, selected managed rulesets, or selected managed rule IDs. Complex rules still expose ref/version, raw action parameters, rate-limit, logging, exposed credential check metadata, and a copyable audit JSON payload for auditing. Zone Analytics uses the Cloudflare Go SDK dashboard endpoint for 24h/7d/30d traffic summaries when `analytics.read` or `account-analytics.read` is granted, including copyable summary export text and compact request trend bars for the returned time series. Account Usage uses backend GraphQL Analytics API queries to show Workers, last-hour Workers errors, R2 operations, D1, and KV usage for the selected account with the same analytics scopes; R2 storage and object count are best-effort overwritten from REST `accounts/{account_id}/r2/metrics` using the same Standard-class snapshot as orange-cloud. Account Usage also makes a best-effort backend call to `accounts/{account_id}/subscriptions` for billing period and Workers/R2 paid-plan context. Cloudflare OAuth normally does not expose a billing scope, so subscription 403 responses are shown as unavailable billing context and do not block usage data. Cloudflare Status uses the public Statuspage API through the cfui backend and does not require an OAuth scope. Zone settings support SSL/TLS mode, TLS 1.3, Minimum TLS Version, development mode, security level, cache level, browser cache TTL, Always Use HTTPS, Automatic HTTPS Rewrites, Brotli, Rocket Loader, IPv6, WebSockets, HTTP/2, HTTP/3, Early Hints, Email Obfuscation, Hotlink Protection, Server Side Excludes, Always Online, Browser Integrity Check, IP Geolocation, Opportunistic Encryption, and 0-RTT when `zone-settings.write` is granted; full cache purge is shown separately when `cache_purge.write` is granted. Native R2 multipart remains follow-up work unless OAuth REST exposes a multipart endpoint or users provide R2 S3 credentials. The UI hides modules when the current OAuth token does not include the required scope.

Zone detail now acts as an orange-cloud-style domain tool hub: when scopes allow it, the selected zone exposes direct entries into DNS, Analytics, WAF, Snippets, and Zone Settings, plus quick actions for development mode, security level, and full cache purge.

OAuth Tunnel ingress panels show the linked local tunnel profile state and include a local workspace action. Remote hostname changes stay in the OAuth console, while starting, stopping, or restarting the local `cloudflared` connector remains in `/local`.

R2 object detail views also expose copy-key, copy-object, and move-object actions, including same-account cross-bucket copies/moves, plus metadata including size, content type, ETag, storage class, last modified time, and encoding status. Non-UTF-8 R2 objects and KV values show a bounded hexdump sample in the detail panel. KV values can also be uploaded and downloaded as raw bytes through the same-origin backend proxy. Inline media/PDF previews are capped at 50 MiB; larger objects stay download-only. The upload form uses direct upload for files up to 128 MiB and switches larger files to chunked browser-to-cfui upload up to 5 GiB.
...
2. Save the OAuth client ID in the OAuth setup page, or set `CFUI_OAUTH_CLIENT_ID` in Docker/Compose. If both exist, the environment variable wins. The provided relay uses client ID `27efa323c59c87976675dc75bcad907b`.
3. Keep `CFUI_OAUTH_RELAY_URL` at the default relay (`https://cfoauth.pushcat.eu.org/oauth/callback`) or point it to your own Worker relay. The OAuth setup page's Default action saves both the provided client ID and this relay URL. You can also edit the relay callback from the OAuth setup page; that saved SQLite value overrides the environment default. The page provides copy, save, and check actions; the check verifies whether the relay `/health` endpoint supports `state-v1` callbacks.
4. Deploy or use the Worker relay from `docs/cloudflare-oauth-worker.js`. The Cloudflare OAuth app redirect URI should be only the Worker's public HTTPS callback URL, for example `https://cfoauth.pushcat.eu.org/oauth/callback`; do not append `cfui_callback_url`. cfui encodes the current browser-facing `/oauth/callback` URL into OAuth `state`, and the Worker reads that state to forward `code` and `state` back to the correct cfui instance. If one Worker serves public cfui domains, configure the Worker variable `CFUI_ALLOWED_CALLBACK_ORIGINS` with a comma-separated cfui origin allowlist, not full callback URLs. For example, use `CFUI_ALLOWED_CALLBACK_ORIGINS=https://cfui.tinycat.heiyu.space` for one exact cfui domain, or `CFUI_ALLOWED_CALLBACK_ORIGINS=https://cfui.*.heiyu.space,https://admin.example.com` for multiple exact/wildcard origins. Each `*` matches exactly one DNS label and cannot replace the last two labels. Intentionally set `*` only for an open multi-user relay. Loopback, private/LAN IPs, `.local`, `.internal`, `.lan`, `.home.arpa`, and `.test` callback hosts are allowed by default. `CFUI_CALLBACK_URL` remains an optional fallback only. The Worker exposes `/health` with a `state-v1` marker, and cfui can check whether the deployed relay is new enough from the OAuth setup page or `GET /api/oauth/relay-check`.
...
```text
account-settings.read zone.read dns.read
```
...
```text
account-settings.read zone.read dns.read dns.write zone-settings.read zone-settings.write cache_purge.write
```
...
Worker script deployment/editing is not part of the current orange-cloud baseline in cfui, so the built-in scope selector does not request `workers-scripts.write`.
...
| `CFUI_OAUTH_CLIENT_ID` | Cloudflare OAuth client ID; overrides the WebUI-saved value when set | unset |
| `CFUI_OAUTH_RELAY_URL` / `CFUI_OAUTH_REDIRECT_URI` | Default OAuth Worker relay callback URL registered in Cloudflare; a WebUI-saved SQLite value overrides it | `https://cfoauth.pushcat.eu.org/oauth/callback` |
| `CFUI_OAUTH_SCOPES` | Default space-separated OAuth scope template used by direct `/oauth/start`; the WebUI starts from the base read scopes and lets users explicitly select more | `account-settings.read zone.read dns.read` |
...
- `GET /api/cf/overview`
- `GET /api/cf/validation`
- `GET /api/cf/validation-reports`
- `POST /api/cf/validation-reports`
- `GET /api/cf/validation-reports/{report_id}`
- `DELETE /api/cf/validation-reports/{report_id}`
- `GET /api/cf/accounts`
- `GET /api/cf/status`
...
- `GET /api/cf/kv/value/download`
- `POST /api/cf/kv/value/upload`
- `GET /api/cf/snippets`
